# ============================
# Fig5 FINAL (ONE FILE ONLY)
# outputs:
#   figs/Fig5_FINAL.pdf
#   figs/Fig5_FINAL.png
# ============================

# ---- auto install missing pkgs ----
...
if(is.na(sample_col) || is.na(arm_col)){
  stop("sample_sheet.csv must have sample/GSM and arm/group/treat columns. Found: ",
...
plot_box_clean <- function(df, features, xlab, title){
  ...
  ggplot(df2, aes(x=score, y=feature, fill=group)) +
    ...
    geom_point(aes(color=group),
               ...
               alpha=0.28, size=1.05, show.legend=FALSE) +
...
# ---- Panel E: group-average composition (NO sample stacking) ----
...
pF <- ggplot(dfF, aes(x=DiD, y=feature, fill=DiD>0)) +
  ...
  theme_jtm +
  theme(plot.margin = margin(6, 26, 6, 8)) +
  coord_cartesian(xlim=c(min(dfF$DiD)*1.35, max(dfF$DiD)*1.35))

# ---- shared legend (single, bottom) ----
...
# ---- SAVE ONLY FINAL ----
ggsave("figs/Fig5_FINAL.pdf", fig5, width=11.5, height=12)
ggsave("figs/Fig5_FINAL.png", fig5, width=11.5, height=12, dpi=300)
