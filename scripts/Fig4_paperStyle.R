...
if (is.na(score_f)) {
  stop("❌ Missing GSVA per-sample matrix. Put one of:\n", paste(cand_score, collapse="\n"),
       "\nFormat: first col pathway, remaining cols sample IDs.")
}
...
cat_path <- function(p){
  p2 <- tolower(p)
  if (str_detect(p2, "keratin|epiderm|cornif|skin|epitheli|barrier|differentiation")) return("Repair")
...
# -------------------------
# patient delta (week1-week0) for each pathway
# -------------------------
...
did_tbl <- did_tbl %>%
  ...
  mutate(
    ...
    pathway_short = short_path(pathway, 40)
  )

write_csv(did_tbl, "figs/Fig4_tables/DiD_pathway_table.csv")

# ============================================================
# Panel A: Forest-style (Top 28 pathways)
# ============================================================
TOP_A <- 28
A_df <- did_tbl %>%
...
col_cat <- c(Repair="#E64B35", Inflammation="#4DBBD5", `ECM/Wnt`="#00A087", Other="grey70")

pA <- ggplot(A_df, aes(y=pathway_short, x=effect, color=category)) +
  geom_vline(xintercept=0, linewidth=0.4, color="grey50") +
  geom_segment(aes(x=0, xend=effect, yend=pathway_short), linewidth=0.9) +
  geom_point(size=2.6) +
...
# ============================================================
# Panel B: Pathway volcano (clean, label Top 6)
# ============================================================
labB <- did_tbl %>%
  filter(FDR < 0.05) %>%
...
pB <- ggplot(did_tbl, aes(effect, neglogFDR, color=category)) +
  ...
  geom_hline(yintercept=-log10(0.05), linetype="dashed", linewidth=0.3, color="grey50") +
...
# ============================================================
# Panel C: Patient-level Δ waterfall (核心：清爽、分栏、对齐)
# 选一个“代表性修复通路”和一个“代表性炎症通路”，再做双行（2 panels）
# ============================================================
# pick pathways by priority keywords; fallback by strongest effect within category
...
C_df <- delta_long %>%
  ...
  mutate(
    pathway_short = short_path(pathway, 32),
...
# x as rank within each arm, keep spacing between arms via facet
pC <- ggplot(C_df, aes(x=rank, y=delta, color=arm)) +
  geom_hline(yintercept=0, linewidth=0.4, color="grey50") +
  geom_segment(aes(xend=rank, y=0, yend=delta), linewidth=0.9) +
  geom_point(size=2.4) +
...
# ============================================================
# Panel D: LOPO robustness (Top 12)
# ============================================================
...
D_df <- tibble(
  pathway = topD,
  pathway_short = short_path(topD, 36),
  prop_consistent = unlist(lopo_prop)
) %>%
...
pD <- ggplot(D_df, aes(x=prop_consistent, y=pathway_short)) +
  geom_vline(xintercept=0.8, linetype="dashed", linewidth=0.3, color="grey50") +
  geom_segment(aes(x=0, xend=prop_consistent, yend=pathway_short), linewidth=0.9, color="grey35") +
...
# ============================================================
# Assemble (paper-style, clear)
# ============================================================
# 左列：A（forest）+ D（robust）
# 右列：B（volcano）+ C（waterfall）
left  <- pA / pD + plot_layout(heights=c(1.15, 0.85))
right <- pB / pC + plot_layout(heights=c(1.0, 1.0))

fig <- left | right
fig <- fig + plot_layout(widths=c(1.05, 1.15))

ggsave("figs/Fig4_paperStyle.pdf", fig, width=14, height=10.5, device="pdf")
ggsave("figs/Fig4_paperStyle.png", fig, width=14, height=10.5, dpi=300)

cat("✅ Saved: figs/Fig4_paperStyle.(png/pdf)\n")
