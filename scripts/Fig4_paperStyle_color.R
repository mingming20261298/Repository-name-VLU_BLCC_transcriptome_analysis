...
# -------------------------
# Palettes (diverse but controlled)
# -------------------------
# Pathway category colors (Panel A/B)
col_pathway <- c(
  "Repair"        = "#D73027",  # red
  "Inflammation"  = "#4575B4",  # blue
  "ECM/Wnt"       = "#1A9850",  # green
  "Other"         = "#7F7F7F"   # grey
)

# Patient-level colors (Panel C): same hue, different tone (w0 light -> w1 dark)
# Names will be built as paste0(arm,"_",time2)
col_patient_template <- function(arm_ctrl, arm_blcc){
  setNames(
    c("#9ECAE1", "#3182BD",  # Control_w0, Control_w1
      "#FCAE91", "#CB181D"), # BLCC_w0, BLCC_w1
    c(paste0(arm_ctrl,"_w0"), paste0(arm_ctrl,"_w1"),
      paste0(arm_blcc,"_w0"), paste0(arm_blcc,"_w1"))
  )
}
...
col_patient <- col_patient_template(arm_ctrl, arm_blcc)
...
# ============================================================
# Panel B: Pathway volcano (label Top 8)
# ============================================================
labB <- did_tbl %>%
  filter(FDR < 0.05) %>%
...
# ============================================================
# Panel C: Patient-level Δ waterfall (分 arm 分栏；颜色同色系不同深浅)
# 选 2 条通路：1 Repair + 1 Inflammation（最像临床主图）
# ============================================================
...
C_df <- delta_long %>%
  ...
  mutate(
    ...
    # color_key：arm_time（实现同色系不同深浅）
    color_key = paste0(as.character(arm), "_", "w1") # placeholder
  ) %>%
  # 加回 time2（用于色调）：waterfall 用 delta，所以 time2 在这里固定为 Δ，但我们用 arm 分色即可
  group_by(pathway_short, arm) %>%
  arrange(desc(delta), .by_group=TRUE) %>%
  mutate(rank = row_number()) %>%
  ungroup()

# waterfall：arm 用两色（更像临床主图），不再用 4 色；4 色保留给 paired/时间图
...
D_df <- tibble(
  pathway = topD,
  pathway_short = short_path(topD, 42),
...
pD <- ggplot(D_df, aes(x=prop_consistent, y=pathway_short)) +
  ...
  geom_segment(aes(x=0, xend=prop_consistent, yend=pathway_short), linewidth=0.95, color="grey40") +
  geom_point(size=2.6, color="grey20") +
...
ggsave("figs/Fig4_paperStyle_color.pdf", fig, width=14, height=10.5, device="pdf")
ggsave("figs/Fig4_paperStyle_color.png", fig, width=14, height=10.5, dpi=300)

cat("✅ Saved: figs/Fig4_paperStyle_color.(png/pdf)\n")
