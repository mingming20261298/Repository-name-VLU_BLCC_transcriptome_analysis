...
tab2 <- tab2 %>%
  left_join(gene_map, by="probe") %>%
  mutate(label_gene = if_else(is.na(gene) | gene=="", probe, gene))
...
# order: keep arm appearance order; time fixed w0->w1
...
row_labels <- tab2 %>%
  filter(probe %in% top_probes) %>%
  select(probe, label_gene) %>%
  distinct(probe, .keep_all=TRUE)
...
# ============================================================
# Panel C: patient-level Δ signature + RIGHT stacked indicators
# (right stacked = per-arm counts of Δ>0 vs Δ<0)
# ============================================================
sig_topk <- 40
sig_tbl <- tab2 %>%
  filter(FDR < FDR_TH, probe %in% rownames(mat)) %>%
  arrange(desc(abs(logFC))) %>%
  slice_head(n=sig_topk)

if(nrow(sig_tbl) < 10) stop("Too few significant probes for signature; loosen FDR_TH or check IDs.")

up_p   <- sig_tbl %>% filter(logFC > 0) %>% pull(probe) %>% unique()
down_p <- sig_tbl %>% filter(logFC < 0) %>% pull(probe) %>% unique()
...
m_sig_z <- t(scale(t(m_sig))); m_sig_z[is.na(m_sig_z)] <- 0
...
pC_main <- ggplot(delta_df2, aes(x=x, y=delta, color=arm)) +
...
# ---- RIGHT stacked indicator (counts of delta sign by arm) ----
ind_df <- delta_df %>%
  mutate(sign = case_when(delta > 0 ~ "Δ>0", delta < 0 ~ "Δ<0", TRUE ~ "Δ=0")) %>%
  count(arm, sign) %>%
  group_by(arm) %>% mutate(arm = factor(arm, levels=arms)) %>% ungroup()

# keep order in legend: Δ>0, Δ<0, Δ=0
ind_df$sign <- factor(ind_df$sign, levels=c("Δ>0","Δ<0","Δ=0"))

pC_right <- ggplot(ind_df, aes(x=arm, y=n, fill=sign)) +
  geom_col(width=0.7) +
  coord_flip() +
  theme_classic(base_size=12) +
  theme(
  ...
  labs() +
  scale_fill_manual(values=c("Δ>0"="#E64B35", "Δ<0"="#4DBBD5", "Δ=0"="grey70"))

pC <- (pC_main | pC_right) + plot_layout(widths=c(4.2, 1.3))
...
ggsave("figs/Fig3_ONE.png", fig, width=13.6, height=9, dpi=300)
ggsave("figs/Fig3_ONE.pdf", fig, width=13.6, height=9, device=cairo_pdf)
